...
해제 버튼은 25 LOAD, 28 ACK, 29 A/B, 30 OPEN, 32 INC/DEC/RESET, 33 RESET, 34 KICK, 37 SUBMIT, 38 WRITE, 39 SAVE/UNDO, 40 SEND/TAKE, 41·42 PUSH/POP, 43 RECEIVE, 45 ADD/RESET이다. 제출 회로의 `inputMode` 대신 신뢰하는 reference가 정한다. 31·35·36·44도 관측하되 해제 목록은 비어 있다. 스위치는 샘플링한 값을 유지하며 해제는 다음 tick이 아니다.

입력만 바꾸면 저장 출력과 RISE·GO·DONE·VALID·ACCEPTED·승인 결과는 다음 실제 tick까지 유지된다. 33은 현재 phase와 현재 DUTY를 비교하므로 DUTY 변경을 즉시 반영한다. 38은 WRITE=0/1 모두에서 현재 ADDR의 기존 저장값을 읽는다. DATA/WRITE 변경은 쓰기가 아니며 실제 쓰기 tick 뒤에 새 값이 나타난다. 관측에서 계산한 임시 nextState는 버리고 실제 tick의 nextState만 탐색에 사용한다.
...
- `npm test`: 초기/SET/완료 tick, 네 추가 반례와 세 기존 반례, 19개 정상 회로, 펄스/EMPTY/33 DUTY/38 WRITE=0·1, 숨은 프레임 미표시, 구형 trace, 매 경계 중단 복원, 관측 중 취소/한도, 버전 3/4 역사 보존.
