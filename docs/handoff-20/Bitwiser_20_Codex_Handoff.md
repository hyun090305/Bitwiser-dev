...
**입력:** `D0`, `D1`, `WRITE`, `COMMIT`, `RESET` → `D0`, `D1`, `WRITE`, `COMMIT`, `RESET`.

**출력:** `Q0`, `Q1` → `Q0`, `Q1`.

**채택 회로:** `C5-04`, 23블록. 준비 레지스터와 공개 레지스터에 각각 D+EN 두 개를 사용합니다. 같은 명세의 18블록 비평면 후보 대신 RESET 배선을 분리한 23블록 평면 회로를 채택합니다.
