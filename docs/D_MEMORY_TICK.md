...
1. D 또는 button 입력이 있는 회로에서는 캔버스 바깥에 재생 바가 나타납니다.
2. 속도 슬라이더는 초당 1~10 tick을 선택합니다. 가장 느린 설정은 1초에 한 tick입니다.
3. **계속**을 누르면 선택한 속도로 재생하며, 같은 버튼이 **일시정지**로 바뀝니다. 편집·swap·채점·화면 이탈·백그라운드 전환도 재생을 멈춥니다.
...
| 항목 | 적용한 계약 |
| --- | --- |
| AND / OR | 편집·엔진·채점 모두 최대 2개로 검증. 기존 3개 이상 입력 파일은 오류를 표시하며 수정 가능. 연결 0개일 때 AND=1, OR=0 유지 |
| NOT | 기존 첫 입력 반전 규칙 유지. 다중 입력을 임의로 금지하거나 새로운 게이트 의미로 바꾸지 않음. 연결 0개는 1 |
| OUTPUT / JUNCTION | 연결 0개는 0. 기존 채점이 2개 이상을 거부했지만 편집/화면 엔진은 허용하던 충돌을 해결해, 이제 편집·엔진·채점 모두 최대 1개로 검증. 기존 다중 입력 파일은 오류를 표시하며 수정 가능 |
| 분기·공간 | 기존 출력 fan-out과 직교·빈 중간 셀·교차 금지 조건 유지 |
| D | 유효 입력 1~2개만 실행 가능. 0개는 미완성, 3개 이상/손상된 역할은 오류 |
| 피드백 | 모든 피드백이 D를 통과하면 허용. 안정점 유무와 관계없이 순수 조합 cycle은 오류 |
