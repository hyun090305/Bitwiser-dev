...
export function validateProgress(raw, levels, budgets, themeIds, { recoverFailedRecords = false } = {}) {
  ...
  const savedGradingVersion = raw.gradingVersion || 1;
  ...
  for (const [key, entry] of Object.entries({...raw.archivedStages, ...raw.stages})) {
    ...
    const next = {};
    if (entry.historicalClear != null) {
      check(typeof entry.historicalClear === 'boolean', 'Invalid historical clear');
      next.historicalClear = entry.historicalClear;
    }
    ...
    if(previousDefinitions.length) {
      const obsolete=['draft','best','bestStars'].some(field=>{
        ...
        try {validateCircuit(entry[field].circuit,id,levels);return false;} catch{return true;}
    ...
    // Preserve previous designs and earned progress; only current, reverified
    // records may supply today's best cost or leaderboard entry.
    const preserveLegacy = record => {
      ...
      return { ...record, circuitVersion: CIRCUIT_VERSION, circuit: validateCircuit(record.circuit, id, levels) };
    };
    ...
    for (const field of ['best', 'bestStars']) {
      if (entry[field]) {
        ...
        const gradingVersion = entry[field].gradingVersion ?? savedGradingVersion;
        check(Number.isInteger(gradingVersion) && gradingVersion >= 1 && gradingVersion <= GRADING_VERSION, 'Unsupported record grading version');
        let verified;
        try {
          check(passesStage(circuit, id, levels), 'Circuit does not pass this stage');
          if (isCurrentCostRecord(entry[field], levels, id)) verified = makeRecord(circuit, id, levels);
        } catch (error) {
          if (gradingVersion >= GRADING_VERSION) {
            if (!recoverFailedRecords) throw error;
            // Local startup must retain unrelated progress even when a record
            // fails revalidation or cannot finish within the grading budget.
            (next.previousCostRecords ||= []).push({ ...entry[field], circuit });
            next.historicalClear = true;
            next.highestStars = Math.max(next.highestStars || 0, id === 0 ? 0 : Math.min(3, Number(entry[field].stars) || 0));
            continue;
          }
        }
        // Keep the old version even after a successful recheck. A container
        // version upgrade is not a new clear, cost conversion, or promotion.
        next[field] = verified || { ...entry[field], gradingVersion, circuit, ...getCircuitStats(circuit) };
        next[field].stars = id === 0 ? 0 : Math.min(3, Math.max(next[field].stars || 1, Number(entry[field].stars) || 1));
      }
    }
    if (recoverFailedRecords) {
    ...
    if (entry.previousCostRecords) {
      ...
      next.previousCostRecords = [...structuredClone(entry.previousCostRecords), ...(next.previousCostRecords || [])];
    }
