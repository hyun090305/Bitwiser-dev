...
for(const [slot,id] of Object.entries(MEMORY20_IDS)) {
  ...
  for(let attempt=0;attempt<attempts;attempt++)circuit=packCircuit(circuit,{seed:id+attempt*7919,rounds:80});
  if(!gradeCircuitSync(circuit,{mode:'sequential',referenceId:`memory20:${slot}`}).ok)throw new Error(`${slot}: grading failed`);
